...
def get_config():
    ...
    parser.add_argument('--use_ema_for_train', type=str2bool, default=False, help='ema_model for train')
    parser.add_argument('--use_mod', type=str2bool, default=False, help='ema_model for train')
    ...
    args = parser.parse_args()
    over_write_args_from_file(args, args.c)
    args.save_name = args.save_name+'+slow_lr:'+str(args.slow_lr)+'+fast_lr:'+str(args.fast_lr)+'+cluster_weight:'+str(args.cluster_weight)+'+ulb_loss_ratio:'+str(args.ulb_loss_ratio)+'+neg_loss_ratio:'+str(args.neg_loss_ratio)+'label_idx_name:'+str(args.label_idx_name)
    return args
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'secu' in args.load_path:
        new_checkpoint = {}
        for name,param in checkpoint['state_dict'].items():
            new_checkpoint[name[7:]] = param
        print(model.model.load_state_dict(new_checkpoint,strict=False))
    elif '.tar' in args.load_path:
        if 'byol' in checkpoint.keys():
            print(model.model.load_state_dict(checkpoint['byol'],strict=False))
        elif 'model' in checkpoint.keys():
            print(model.model.load_state_dict(checkpoint['model'],strict=False))
        else:
            print(model.model.load_state_dict(checkpoint,strict=False))
    elif '.pth' in args.load_path:
        param_dict = {}
        state_dict=checkpoint['model']
        for key in state_dict.keys():
            if 'cluster_head' in key:
                continue
            param_dict.update({key[7:]:state_dict[key]})
        print(model.model.load_state_dict(param_dict,strict=False))
    else:
        print(model.model.load_state_dict(checkpoint,strict=False))
