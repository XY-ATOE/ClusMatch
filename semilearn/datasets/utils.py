...
def split_ssl_data(args, data, targets, num_classes,
    ...
    lb_idx, ulb_idx = sample_labeled_unlabeled_data_from_selfsup(args, data, targets, num_classes, 
    ...
    if include_lb_to_ulb:
        ulb_idx = np.array(range(len(targets)))
    print(targets)
...
def sample_labeled_unlabeled_data_from_selfsup(args, data, target, num_classes,
                                  lb_num_labels, ulb_num_labels=None,
                                  lb_imbalance_ratio=1.0, ulb_imbalance_ratio=1.0,
                                  load_exist=True):
    '''
    ...
    dump_dir = os.path.join(base_dir, 'data', args.dataset, 'labeled_idx')
    os.makedirs(dump_dir, exist_ok=True)
    lb_dump_path = os.path.join(dump_dir, args.label_idx_name)
    print(lb_dump_path)
    if os.path.exists(lb_dump_path) and load_exist:
        lb_idx = np.load(lb_dump_path)
        num_elems = len(target)
        ulb_idx=np.setdiff1d(range(num_elems), lb_idx)
        print(len(ulb_idx))
        return lb_idx,ulb_idx
    else:
        print("labeled idx not exit")
