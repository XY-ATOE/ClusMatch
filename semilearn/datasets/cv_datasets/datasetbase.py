...
class BasicDataset(Dataset):
    ...
    def updatate_transform(self,transform):
        self.transform=transform
    def init_ori_data(self):
        self.ori_data = self.data
        self.ori_targets=self.targets
    
    def update(self,mark):
        index=torch.nonzero(mark>0.5).squeeze().cpu().numpy()
        self.data ,self.targets = self.ori_data[index],self.ori_targets[index]
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                if self.strong_transform is None:
                    ...
                    strong_img={}
                    for i in range(20):
                        strong_img['x_lb_s'+str(i)]=self.strong_transform(img)
                    need_return = {'idx_lb': idx, 'x_lb': img_w, 'y_lb': target} 
                    need_return.update(strong_img)
                    return need_return
