...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset,get_GCC_imagenet,get_stl10_GCC
    ...
    if dataset == "eurosat":
        ...
        lb_dset, ulb_dset, eval_dset = get_stl10_GCC(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
        ...
        test_dset = None
    elif dataset in ["imagenet", "imagenet-10", "imagenet-dogs",'tiny-imagenet-200-new','tiny-imagenet-200']:
        lb_dset, ulb_dset, eval_dset = get_GCC_imagenet(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    # speech dataset
...
def get_optimizer(net, optim_name='SGD', fast_lr=0.1,slow_lr=0.1, momentum=0.9, weight_decay=0, layer_decay=1.0, nesterov=True, bn_wd_skip=True):
    '''
    ...
    if layer_decay != 1.0:
        ...
        if hasattr(net,'encoder_k'):
            per_param_args_1 = param_groups_weight_decay(net.encoder_k, weight_decay, no_weight_decay_list=no_decay,lr=slow_lr)
            per_param_args_2 = param_groups_weight_decay(net.projector_k, weight_decay, no_weight_decay_list=no_decay,lr=fast_lr)#0.0002
            per_param_args_3 = param_groups_weight_decay(net.cluster_head, weight_decay, no_weight_decay_list=no_decay,lr=fast_lr)#0.02
            per_param_args=per_param_args_1+per_param_args_2+per_param_args_3
        elif hasattr(net,'backbone'):
            per_param_args_1 = param_groups_weight_decay(net.backbone, weight_decay, no_weight_decay_list=no_decay,lr=slow_lr)#0.0020
            #per_param_args_2 = param_groups_weight_decay(net.contrastive_head, weight_decay, no_weight_decay_list=no_decay,lr=slow_lr*100)#0.0020
            per_param_args_3 = param_groups_weight_decay(net.cluster_head_new, weight_decay, no_weight_decay_list=no_decay,lr=fast_lr)#0.01
            #per_param_args=per_param_args_1+per_param_args_2+per_param_args_3
            per_param_args=per_param_args_1+per_param_args_3
        elif hasattr(net,'encoder'):
            per_param_args_1 = param_groups_weight_decay(net.encoder, weight_decay, no_weight_decay_list=no_decay,lr=slow_lr)#0.0020
            per_param_args_2 = param_groups_weight_decay(net.predictor, weight_decay, no_weight_decay_list=no_decay,lr=fast_lr)#0.0020
            per_param_args_3 = param_groups_weight_decay(net.cluster_head_new, weight_decay, no_weight_decay_list=no_decay,lr=fast_lr)#0.01
            # per_param_args_3 = [{'params': net.center_0 , 'weight_decay': weight_decay,'lr':fast_lr}]
            #per_param_args=per_param_args_1+per_param_args_2+per_param_args_3
            per_param_args=per_param_args_1+per_param_args_3
        
    if optim_name == 'SGD':
        optimizer = torch.optim.SGD(per_param_args, lr=fast_lr, momentum=momentum, weight_decay=weight_decay,
                                    nesterov=nesterov)
    elif optim_name == 'AdamW':
        optimizer = torch.optim.AdamW(per_param_args, lr=fast_lr, weight_decay=weight_decay)
